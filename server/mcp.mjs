...
// Owner rule (2026-09-07): only FULL games. No game arrives as a link — chimpvibe_submit_game refuses and explains.
export const HOSTING_RULE = "ChimpVibe hosts every game itself, on its own servers, and members never host their own: a game that lives on someone else's server can go dark, cannot be forked, carries no tree and no tags, and nobody but its author can fix it — so a link is not a game here. What ChimpVibe accepts is CODE that runs on one of its hosts: fork a hosted game (chimpvibe_games → chimpvibe_tree → chimpvibe_fork_from, or a pasted tag) and submit your change; it becomes a fruit the owner can deploy, and every member can fork it in turn. A brand-new game gets in the same way — as a proposal on the closest hosted game (Ssnake and Lumen Coil run the Snake Evolve engine; a proposal may replace the game inside game/ wholesale, as Lumen Coil once did) — never as a link.";
...
export function createMcp({ dataRoot, catalogNow, readTree, guideText, version = '1.0', gameMcpUrl = (p) => p.fork?.mcp || null }) {
  ...
  const tools = [
    ...
    { name: 'chimpvibe_submit_game', description: 'NOT ACCEPTED any more — ChimpVibe takes no game by link: it hosts every game itself and members never host their own. Calling this returns the rule and the way in (a fork of a hosted game). A brand-new game is submitted as CODE: a proposal on the closest hosted game.', inputSchema: { type: 'object', properties: { title: { type: 'string', maxLength: 80 }, blurb: { type: 'string', maxLength: 400 }, host: { type: 'string', description: 'https URL of the playable game' }, repo: { type: 'string', description: 'optional https URL of the source' }, art_png_base64: { type: 'string', description: 'optional PNG capture, base64' } }, required: ['title', 'blurb', 'host'], additionalProperties: false } },
  ...
  async function call(name, args, who) {
    ...
    switch (name) {
      ...
      case 'chimpvibe_submit_game': {
        // Owner rule (2026-09-07): refused, always, with the reason and the way in — nothing is written.
        return fail(`REJECTED — SUBMISSION_BY_LINK_NOT_ACCEPTED. ${HOSTING_RULE} Next: chimpvibe_games {} → pick the hosted game closest to yours → chimpvibe_tree → chimpvibe_fork_from {ref} → the begin call → put your game's code under game/ → validate → submit_proposal. Tell your user plainly that ChimpVibe does not take links and why.`);
      }
      case 'chimpvibe_submit_game__retired': {
  ...
  async function rpc(message, who) {
    ...
    switch (method) {
      case 'initialize': {
        ...
        return reply({ protocolVersion: PROTOCOLS.includes(asked) ? asked : PROTOCOLS[0], capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'chimpvibe', version }, instructions: 'ChimpVibe is a site where a community branches and forks games in the open. This ONE server fronts every game with your one token. RULE: ChimpVibe hosts every game itself; members never host their own — no game is accepted as a link, only as code (a proposal on a hosted game); chimpvibe_submit_game refuses and says why. If the user pasted a TAG (chimpvibe:<slug>#<n>, e.g. chimpvibe:ssnake#9): call chimpvibe_resolve {tag} FIRST and then offer exactly three choices — Install <game> · Ask about <game> · Modify <game> — and wait for one (its reply spells each out). Otherwise call chimpvibe_whoami first, then chimpvibe_guide; to change a game: chimpvibe_tree → chimpvibe_fork_from {ref} → the begin call it returns → patch → validate → submit. If you have the ChimpVibe skill, run /chimpvibe:contribute — it walks you through every step. After you submit, your job is OVER: the owner alone reviews and deploys (nothing is public before that) — never publish, host or deploy anything yourself; tell your user "Submission complete, there\'s no more for me to do." and stop.' });
      }
